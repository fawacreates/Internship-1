# Week 1: Programming Core + Math Refresh - Progress Log

**Date:** July 22, 2026
**Week Focus:** Python basics, control flow, functions

## Daily Progress

### Monday (Day 1)
- [x] Completed CS50P Lecture 0 (Functions & Variables)
- [x] Wrote hello.py
- [x] Wrote calculator.py
- **Time:** 2 hours
- **Notes:** 

### Tuesday (Day 2)
- [x] Completed CS50P Lecture 1 (Conditionals)
- [x] Wrote compare.py
- [x] Wrote grade.py
- **Time:** 2 hours
- **Notes:** 

### Wednesday (Day 3)
- [ ] Completed CS50P Lecture 2 (Loops)
- [ ] Wrote cat.py
- [ ] Wrote parity.py
- **Time:** 2 hours
- **Notes:** 

### Thursday (Day 4)
- [ ] Reviewed Python 3.10 match/case statement
- [ ] Wrote house.py
- [ ] Studied define.py patterns
- **Time:** 2 hours
- **Notes:** 

### Friday (Day 5)
- [ ] Code review and cleanup
- [ ] Pushed all scripts to GitHub
- [ ] Created this learning log
- **Time:** 1.5 hours
- **Notes:** 

### Saturday (Day 6)
- [ ] Studied list comprehensions
- [ ] Started NumPy basics
- **Time:** 2 hours
- **Notes:** 

### Sunday (Day 7)
- [ ] Weekly reflection
- [ ] Plan for Week 2
- **Time:** 1 hour
- **Notes:** 

## Key Learnings

### Python Concepts Mastered
-  Variables and data types
- Functions and parameters
-  If/elif/else conditionals
-  While and for loops
-  Boolean logic
-  Match/case pattern matching (Python 3.10+)
-  List comprehensions (in progress)

### Challenges Faced
- 

### Resources Used
- CS50P by David Malan
- Python documentation
- PyCharm IDE

## Time Allocation This Week
```
Hands-on coding:     40% (8 hours)
Structured learning: 30% (6 hours)
Project work:        20% (4 hours)
Documentation:       10% (2 hours)
Total:              20 hours
```

## Week 1 Deliverables
-  8 Python scripts (calculator, hello, define, compare, grade, parity, cat, house)
-  Pushed to GitHub
-  Basic learning log started

## Next Week (Week 2) Preview

**Topics:** Linux, Bash, Aerospace Fundamentals
- Linux command line basics
- Bash scripting
- Flight dynamics fundamentals
- Coordinate systems
- **Deliverable:** Bash script collection + aerospace notes

## Reflection

**What went well:**
- Strong grasp of Python fundamentals
- Consistent daily practice
- Good progression through CS50P

**What to improve:**
- Need to practice more algorithmic thinking
- Should explore edge cases more thoroughly
- Time management - could batch similar tasks

**Confidence Level:** 8/10 on Python basics

**Ready for Week 2?** YES 
